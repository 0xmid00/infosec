...
```shell-session
0xmid00@htb[/htb]$ git clone https://github.com/tmux-plugins/tpm ~/.tmux/plugins/tpm
```
...
```shell-session
0xmid00@htb[/htb]$ touch .tmux.conf
```
...
```shell-session
0xmid00@htb[/htb]$ tmux source ~/.tmux.conf 
```
...
```shell-session
0xmid00@htb[/htb]$ mkdir -p ACME-IPT/{Admin,Deliverables,Evidence/{Findings,Scans/{Vuln,Service,Web,'AD Enumeration'},Notes,OSINT,Wireless,'Logging output','Misc Files'},Retest}
```

Notetaking & Organization

```shell-session
0xmid00@htb[/htb]$ tree ACME-IPT/
...
```shell-session
0xmid00@htb[/htb]$ xfreerdp /v:10.129.203.82 /u:htb-student /p:HTB_@cademy_stdnt!
```
