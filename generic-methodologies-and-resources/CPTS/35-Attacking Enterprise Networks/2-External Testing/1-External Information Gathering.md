...
```shell-session
0xmid00@htb[/htb]$ sudo nmap --open -oA inlanefreight_ept_tcp_1k -iL scope 
...
```shell-session
0xmid00@htb[/htb]$ sudo nmap --open -p- -A -oA inlanefreight_ept_tcp_all_svc -iL scope
...
```shell-session
0xmid00@htb[/htb]$ egrep -v "^#|Status: Up" inlanefreight_ept_tcp_all_svc.gnmap | cut -d ' ' -f4- | tr ',' '\n' | \                                                               
...
```shell-session
0xmid00@htb[/htb]$ dig axfr inlanefreight.local @10.129.203.101
...
```shell-session
0xmid00@htb[/htb]$ curl -s -I http://10.129.203.101 -H "HOST: defnotvalid.inlanefreight.local" | grep "Content-Length:"
...
```shell-session
0xmid00@htb[/htb]$ ffuf -w namelist.txt:FUZZ -u http://10.129.203.101/ -H 'Host:FUZZ.inlanefreight.local' -fs 15157
...
```shell-session
0xmid00@htb[/htb]$ sudo tee -a /etc/hosts > /dev/null <<EOT
